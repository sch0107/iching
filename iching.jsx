...
const T = {
  "zh-Hans": {
    subtitle:"周易占卜", title:"易经起卦",
    questionLabel:"所 问 之 事（可留空）", questionPlaceholder:"在此输入你想占问的事情…",
    castingText:(s)=>`正在摇卦… 第 ${s} 爻`, castBtn:"☯ 起 卦",
    primaryLabel:"本　卦", resultLabel:"变　卦", changingTitle:"变 爻 详 情",
    yangLine:"老阳 ○", yinLine:"老阴 ✕", yangChange:"阳极变阴", yinChange:"阴极变阳",
    copyHint:"── 复制或导出，粘贴给 AI 解读 ──",
    copyBtn:"复制卦象", copiedBtn:"已复制！", exportBtn:"导出 .txt",
    previewLabel:"预 览", resetBtn:"重 新 起 卦", questionDisplay:"所问之事",
    lineLabels:["初","二","三","四","五","上"], hexNum:(n)=>`第${n}卦`,
    summaryHeader:"【易经起卦结果】", timeLabel:"占卜时间：", questionPrefix:"所问之事：",
    primaryPrefix:"本卦：", judgmentLabel:"卦辞：", changingPrefix:"变爻：",
    resultPrefix:"变卦：", footer:"请根据以上卦象，结合所问之事给出解读。",
    yangChgText:"老阳○（阳极变阴）", yinChgText:"老阴✕（阴极变阳）",
    filePrefix:"卦象",
  },
  "zh-Hant": {
    subtitle:"周易占卜", title:"易經起卦",
    questionLabel:"所 問 之 事（可留空）", questionPlaceholder:"在此輸入你想占問的事情…",
    castingText:(s)=>`正在搖卦… 第 ${s} 爻`, castBtn:"☯ 起 卦",
    primaryLabel:"本　卦", resultLabel:"變　卦", changingTitle:"變 爻 詳 情",
    yangLine:"老陽 ○", yinLine:"老陰 ✕", yangChange:"陽極變陰", yinChange:"陰極變陽",
    copyHint:"── 複製或匯出，貼給 AI 解讀 ──",
    copyBtn:"複製卦象", copiedBtn:"已複製！", exportBtn:"匯出 .txt",
    previewLabel:"預 覽", resetBtn:"重 新 起 卦", questionDisplay:"所問之事",
    lineLabels:["初","二","三","四","五","上"], hexNum:(n)=>`第${n}卦`,
    summaryHeader:"【易經起卦結果】", timeLabel:"占卜時間：", questionPrefix:"所問之事：",
    primaryPrefix:"本卦：", judgmentLabel:"卦辭：", changingPrefix:"變爻：",
    resultPrefix:"變卦：", footer:"請根據以上卦象，結合所問之事給出解讀。",
    yangChgText:"老陽○（陽極變陰）", yinChgText:"老陰✕（陰極變陽）",
    filePrefix:"卦象",
  },
  "en": {
    subtitle:"I Ching Divination", title:"I Ching",
    questionLabel:"Your Question (optional)", questionPlaceholder:"Enter the matter you wish to consult…",
    castingText:(s)=>`Casting line ${s} of 6…`, castBtn:"☯ Cast",
    primaryLabel:"Primary", resultLabel:"Derived", changingTitle:"Changing Lines",
    yangLine:"Old Yang ○", yinLine:"Old Yin ✕", yangChange:"Yang → Yin", yinChange:"Yin → Yang",
    copyHint:"── Copy or export to paste into an AI ──",
    copyBtn:"Copy Reading", copiedBtn:"Copied!", exportBtn:"Export .txt",
    previewLabel:"Preview", resetBtn:"New Reading", questionDisplay:"Question",
    lineLabels:["1st","2nd","3rd","4th","5th","Top"], hexNum:(n)=>`Hex. ${n}`,
    summaryHeader:"[I Ching Reading]", timeLabel:"Date: ", questionPrefix:"Question: ",
    primaryPrefix:"Primary Hexagram: ", judgmentLabel:"Judgment: ", changingPrefix:"Changing Lines:",
    resultPrefix:"Derived Hexagram: ", footer:"Please interpret this reading in light of the question above.",
    yangChgText:"Old Yang ○ (yang → yin)", yinChgText:"Old Yin ✕ (yin → yang)",
    filePrefix:"reading",
  },
};
...
function getHex(arr) {
  return HEXAGRAM_DB[arr.join("")] || {
    ...
    en:{name:"Unknown",title:"Unknown Hexagram",judgment:"This hexagram is not in the database."},
  };
}
function upperTriKey(vals) { return [vals[5],vals[4],vals[3]].join(""); }
function lowerTriKey(vals)  { return [vals[2],vals[1],vals[0]].join(""); }
function hexLang(entry, lang) { return entry[lang==="en"?"en":lang==="zh-Hant"?"hant":"hans"]; }

function buildSummary(question, lines, lang) {
  const t = T[lang];
  ...
  const hexStr     = (entry, d) => lang==="en"
    ? `${d.name} (${t.hexNum(entry.num)} · ${d.title})`
    : `${d.title}（${t.hexNum(entry.num)} · ${d.name}）`;

  let out = t.summaryHeader + "\n";
  out += t.timeLabel + new Date().toLocaleString(lang==="en"?undefined:lang==="zh-Hant"?"zh-TW":"zh-CN") + "\n";
  if (question) out += t.questionPrefix + question + "\n";
  out += "\n" + t.primaryPrefix + hexStr(pEntry, primary) + "\n";
  out += block(lines) + "\n" + t.judgmentLabel + primary.judgment + "\n";
  if (hasChg && result) {
    const chgs = lines.map((v,i)=>(v===6||v===9)
      ? `  ${t.lineLabels[i]}${lang==="en"?" line":" 爻"}：${v===9?t.yangChgText:t.yinChgText}`
      : null).filter(Boolean);
    out += "\n" + t.changingPrefix + "\n" + chgs.join("\n") + "\n";
    out += "\n" + t.resultPrefix + hexStr(rEntry, result) + "\n";
    out += block(changed.map(v=>v===1?7:8)) + "\n" + t.judgmentLabel + result.judgment + "\n";
  }
  out += "\n---\n" + t.footer;
  return out;
}

function LineDraw({ v, idx, showMark, isResult, lineLabels }) {
  const val     = lineVal(v);
...
function HexCard({ label, lines, isResult, lang }) {
  const t      = T[lang];
  const vals   = lines.map(lineVal);
  const entry  = getHex(vals);
  const hex    = hexLang(entry, lang);
  const triRaw = TRIGRAMS[upperTriKey(vals)];
  const trlRaw = TRIGRAMS[lowerTriKey(vals)];
  const lKey   = lang==="en"?"en":lang==="zh-Hant"?"hant":"hans";
  const tu     = triRaw?triRaw[lKey]:{name:"",nature:""};
  const tl     = trlRaw?trlRaw[lKey]:{name:"",nature:""};
  ...
  return (
    <div style={{border:`1px solid ${border}`,background:"rgba(200,168,75,0.03)",padding:"28px 28px 24px",minWidth:200,textAlign:"center"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,margin:"8px 0"}}>
        {[...lines].reverse().map((v,i)=>
          <LineDraw key={i} v={v} idx={i} showMark={!isResult} isResult={isResult} lineLabels={t.lineLabels}/>
        )}
      </div>
      ...
      <div style={{fontSize:11,letterSpacing:2,color:"rgba(200,168,75,0.6)",marginBottom:12}}>
        {t.hexNum(entry.num)} · {hex.title}
      </div>
      <div style={{fontSize:11,color:"rgba(200,168,75,0.5)",lineHeight:2,borderTop:"1px solid rgba(200,168,75,0.1)",paddingTop:12,maxWidth:180,margin:"0 auto"}}>
        {hex.judgment}
      </div>
    </div>
  );
...
export default function IChing() {
  ...
  const summary      = done ? buildSummary(question, lines, lang) : "";
  ...
  const handleExport = () => {
    ...
    a.href=url; a.download=`${t.filePrefix}_${new Date().toISOString().slice(0,10)}.txt`; a.click();
  ...
  const langBtnStyle = (active) => ({
    background:active?"rgba(200,168,75,0.15)":"none",
    border:`1px solid ${active?"rgba(200,168,75,0.5)":"rgba(200,168,75,0.2)"}`,
    color:active?"#f5e09a":"rgba(200,168,75,0.5)",
  ...
  return (
    <div style={{minHeight:"100vh",background:"#150f05",
      ...
      <style>{`
        ...
        .lang-btn:hover{color:rgba(200,168,75,0.8)!important}
      `}</style>

      {/* Language switcher */}
      <div style={{position:"fixed",top:16,right:20,display:"flex",gap:6,zIndex:100}}>
        {[["zh-Hans","简"],["zh-Hant","繁"],["en","EN"]].map(([l,label])=>(
          <button key={l} className="lang-btn" onClick={()=>setLang(l)} style={langBtnStyle(lang===l)}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div style={{textAlign:"center",marginBottom:40}}>
        <div style={{fontSize:10,letterSpacing:8,color:"#c8a84b",opacity:0.6,marginBottom:10}}>{t.subtitle}</div>
      ...
      {!done && (
        <div style={{width:"100%",maxWidth:480,marginBottom:32,animation:"fi 0.5s ease"}}>
          ...
          <textarea value={question} onChange={e=>setQuestion(e.target.value)}
            placeholder={t.questionPlaceholder} rows={3} disabled={casting}
      ...
      {done && question && (
        <div style={{marginBottom:28,maxWidth:520,width:"100%",
          ...
          <div style={{fontSize:10,letterSpacing:4,color:"rgba(200,168,75,0.5)",marginBottom:6}}>{t.questionDisplay}</div>
      ...
      {lines && lines.length>0 && (
        <div style={{display:"flex",gap:32,flexWrap:"wrap",
          ...
          <HexCard label={t.primaryLabel} lines={lines} isResult={false} lang={lang}/>
          {hasChange && lines.length===6 && <>
            <div style={{display:"flex",alignItems:"center",fontSize:24,color:"rgba(200,168,75,0.4)"}}>→</div>
            <HexCard label={t.resultLabel} lines={changedLines} isResult={true} lang={lang}/>
          </>}
        </div>
      )}
      ...
      {done && hasChange && (
        <div style={{marginTop:24,maxWidth:420,width:"100%",
          ...
          <div style={{fontSize:10,letterSpacing:5,color:"#c8a84b",marginBottom:12}}>{t.changingTitle}</div>
          {lines.map((v,i)=>(v!==6&&v!==9)?null:(
            <div key={i} style={{display:"flex",gap:12,alignItems:"center",marginBottom:8,fontSize:13}}>
              <span style={{color:"#c8a84b",minWidth:36}}>{t.lineLabels[i]}{lang==="en"?" line":"爻"}</span>
              <span style={{fontSize:10,color:v===9?"#e8a04b":"#8ab4d4"}}>{v===9?t.yangLine:t.yinLine}</span>
              <span style={{fontSize:10,color:"rgba(200,168,75,0.45)"}}>{v===9?t.yangChange:t.yinChange}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {done && (
        <div style={{marginTop:32,display:"flex",flexDirection:"column",
          ...
          <div style={{fontSize:11,color:"rgba(200,168,75,0.4)",letterSpacing:3}}>{t.copyHint}</div>
          <div style={{display:"flex",gap:12,flexWrap:"wrap",justifyContent:"center"}}>
            <button className="action-btn" onClick={handleCopy} style={btnStyle(copied)}>
              <span style={{fontSize:15}}>{copied?"✓":"⎘"}</span>
              <span style={{minWidth:64}}>{copied?t.copiedBtn:t.copyBtn}</span>
            </button>
          ...
          <div style={{width:"100%",background:"rgba(0,0,0,0.35)",
            ...
            <div style={{fontSize:9,letterSpacing:4,color:"rgba(200,168,75,0.3)",marginBottom:10}}>{t.previewLabel}</div>
